---
title: "Claude Codeのagents / skills / hooksをどう使い分ける？実プロダクト開発で出した運用ルール"
emoji: "🧭"
type: "tech"
topics: ["claudecode", "ai", "flutter", "hooks", "agent"]
published: true
---
...
![インストール用QRコード](/images/claude-code-agents-skills-hooks/apk_QR.jpg)
...
そこで、公開されている評価の高いハーネスエンジニアリングの事例（[affaan-m/ecc](https://github.com/affaan-m/ecc)）を見本に、自分の運用に取り込めるところがないかを探しました。これまで自分は `CLAUDE.md` と skills 止まりだったので、まだ使えていない agents と hooks を理解して取り込めば伸びしろがあると考えたからです。
...
| 仕組み | 実体 | いつ発火するか | 文脈（コンテキスト） |
|---|---|---|---|
| CLAUDE.md | 常時読み込まれるルール文書 | 常に（毎ターン読まれる） | メインの会話と共有 |
| skill | 必要時に読み込む手順書 | モデルまたは人が必要と判断して呼んだ時 | メインの会話と共有 |
| サブエージェント | 独立したClaudeインスタンス（役割定義） | 明示的に作業を委譲した時 | 独立（共有されない） |
| hook | ハーネスが実行するスクリプト | 設定したイベント発生時に必ず（決定論的） | スクリプトの入出力のみ |

押さえておきたいのは次の2点です。

1. `CLAUDE.md` は「常に効くが薄い」、skill は「呼ばれた時だけ効くが厚い」という関係にあります。重い手順を全部 `CLAUDE.md` に書くと毎ターンの文脈を圧迫し、肝心な指示がかすみます。逆に「必ず守ってほしい発火条件」を skill だけに書くと、その skill が呼ばれない限り効きません。
2. サブエージェントだけは文脈が独立しています。メインの会話の情報を引き継がない代わりに、メインの文脈を汚しません。その代わり、ユーザーへの質問（`AskUserQuestion`）や他のエージェントの起動はできない、という制約があります。
...
1. `CLAUDE.md` で共通ルールを言語化する（まず横断ルールを文章にする）
2. 繰り返す手順を skill にする（再現性を上げる）
3. 役割を agents に切り出す（文脈を分ける・並行させる）
4. 決定論的な強制を hook にする（ブレを消す）
